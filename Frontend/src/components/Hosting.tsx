  ...
  return (
    <div
      className={`bg-white rounded-2xl shadow-xl p-8 transition-all hover:shadow-2xl duration-300 flex flex-col ${
        highlight ? 'ring-4 ring-brand-600 scale-105' : ''
      }`}
      data-animate="false"
      data-delay={delayValue}
    >
      <div className="text-center mb-6">
        <h3 className="text-2xl font-bold text-ink-900 mb-2">{name}</h3>
        <p className="text-ink-600 mb-4">{description}</p>
        <div className="mb-2">
          <span className="text-4xl font-bold text-brand-600">{price}</span>
          <span className="text-ink-600 ml-2">/ mois</span>
        </div>
      ...
      <button
        ...
        className={`w-full py-3 px-6 rounded-lg font-semibold transition-all mt-auto ${
          highlight
            ...
            : 'bg-ink-100 text-ink-900 hover:bg-ink-200 active:scale-95'
        }`}
      >
        Choisir {name}
...
const AddOnRow = ({ icon, name, description, price, priceAlt }: AddOnRowProps) => (
  <div className="flex items-start justify-between gap-4 p-4 bg-white rounded-xl border border-ink-200 hover:border-brand-300 transition-colors">
    ...
    <div className="text-right flex-shrink-0">
      <span className="font-bold text-brand-600 whitespace-nowrap">{price}</span>
      {priceAlt && (
        <>
          <span className="block text-xs text-ink-400">ou</span>
          <span className="block font-bold text-brand-600 whitespace-nowrap">{priceAlt}</span>
        </>
      )}
    </div>
  </div>
    ...
    <section className="py-20 bg-ink-50 pt-32" ref={sectionRef}>
      <div className="container mx-auto px-6">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-ink-900 mb-4">Offres d'Hébergement & Maintenance</h1>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-16 max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-ink-200">
            <MapPin className="w-4 h-4 text-brand-600" aria-hidden="true" />
            <span className="text-sm font-medium text-ink-700">Hébergé en France &mdash; données souveraines RGPD</span>
          </div>
          <div className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-ink-200">
            <Activity className="w-4 h-4 text-brand-600" aria-hidden="true" />
            <span className="text-sm font-medium text-ink-700">99% de disponibilité visée</span>
          </div>
          <div className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-ink-200">
            <Shield className="w-4 h-4 text-brand-600" aria-hidden="true" />
            <span className="text-sm font-medium text-ink-700">Sauvegardes quotidiennes chiffrées</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 md:p-12 max-w-5xl mx-auto mb-12">
          <h2 className="text-3xl font-bold text-ink-900 mb-2 text-center">Options à la carte</h2>
          <p className="text-ink-600 text-center mb-8">À ajouter à n'importe quelle formule selon vos besoins.</p>
          {/* Trois cartes dans une grille à deux colonnes : la troisième
              restait collée à gauche, avec un vide à sa droite. Elle occupe
              maintenant les deux colonnes et se recentre à la largeur d'une
              seule, ce qui referme la composition.
              `[&>*:last-child]` cible le dernier enfant sans avoir à passer une
              classe à chaque AddOnRow. */}
          <div className="grid gap-4 md:grid-cols-2 md:[&>*:last-child]:col-span-2 md:[&>*:last-child]:mx-auto md:[&>*:last-child]:w-[calc(50%-0.5rem)]">
            <AddOnRow
              icon={<RotateCcw className="w-5 h-5 text-brand-600" aria-hidden="true" />}
            ...
            />
            <AddOnRow
              icon={<Database className="w-5 h-5 text-brand-600" aria-hidden="true" />}
            ...
            />
            <AddOnRow
              icon={<GraduationCap className="w-5 h-5 text-brand-600" aria-hidden="true" />}
              name="Scan de sécurité"
              description="Détection de vulnérabilités, analyse des en-têtes HTTP, scan des dépendances, tests d'intrusion ciblés. Vous recevez un rapport lisible, avec les actions correctives classées par priorité — pas une sortie d'outil brute. En ponctuel pour un état des lieux, ou en mensuel pour un suivi continu."
              price="25€ une fois"
              priceAlt="15€/mois"
            />
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 md:p-12 max-w-5xl mx-auto mb-12">
          <h2 className="text-3xl font-bold text-ink-900 mb-8 text-center">Pourquoi choisir nos offres d'hébergement ?</h2>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="flex items-start space-x-4">
              <div className="bg-brand-100 p-3 rounded-lg">
                <Server className="w-6 h-6 text-brand-600" aria-hidden="true" />
              </div>
              <div>
                <h3 className="text-xl font-semibold text-ink-900 mb-2">Infrastructure Professionnelle</h3>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div>
                <h3 className="text-xl font-semibold text-ink-900 mb-2">Sécurité Maximale</h3>
                <p className="text-ink-600">
            ...
            <div className="flex items-start space-x-4">
              <div className="bg-brand-100 p-3 rounded-lg">
                <Zap className="w-6 h-6 text-brand-600" aria-hidden="true" />
              </div>
              <div>
                <h3 className="text-xl font-semibold text-ink-900 mb-2">Sauvegarde Quotidienne</h3>
                <p className="text-ink-600">
            ...
            <div className="flex items-start space-x-4">
              <div className="bg-brand-100 p-3 rounded-lg">
                <BarChart className="w-6 h-6 text-brand-600" aria-hidden="true" />
              </div>
              <div>
                <h3 className="text-xl font-semibold text-ink-900 mb-2">Suivi & Rapports</h3>
                <p className="text-ink-600">
        ...
        <div className="bg-brand-50 rounded-2xl p-8 md:p-12 max-w-5xl mx-auto mb-12">
          <h2 className="text-2xl font-bold text-ink-900 mb-6 text-center">Questions fréquentes</h2>
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-ink-900 mb-2">C'est quoi une "petite modification" ?</h3>
              <p className="text-ink-600">
            ...
            <div>
              <h3 className="text-lg font-semibold text-ink-900 mb-2">C'est quoi une "grosse modification" ?</h3>
              <p className="text-ink-600">
            ...
            <div>
              <h3 className="text-lg font-semibold text-ink-900 mb-2">Vous reprenez mon site déjà existant ?</h3>
              <p className="text-ink-600">
            ...
            <div>
              <h3 className="text-lg font-semibold text-ink-900 mb-2">Puis-je changer d'offre plus tard ?</h3>
              <p className="text-ink-600">
